...
class S256Point(Point):
	...
	@classmethod #クラスメソッド化することにより、クラスから直接メソッドを呼び出せる
	def parse(self, sec_bin):
		#16進でないSECバイナリからPointオブジェクトを返す
		if sec_bin[0] == 4:
			x = int.from_bytes(sec_bin[1:33],'big')
			y = int.from_bytes(sec_bin[34:65],'big')
			return S256Point(x=x, y=y)
		#y座標の偶奇は最初のバイトで判断、is_evenにはブール値が入る
		is_even = sec_bin[0] == 2
		...
		if beta.num % 2 == 0:
			even_beta = beta
			odd_data = S256Field(P - beta.num)
		else:
			even_beta = S256Field(P - beta.num)
			odd_data = beta
		if is_even:
			return S256Point(x, even_data)
		else:
			return S256Point(x, odd_data)
...
class Signature:
	...
	def der(self):
		...
		#rbinの最上位ビットが1の場合、\x00を追加(0x80 = 0b1000000)
		if rbin[0] & 0x80:
			rbin = b'\x00' + rbin
